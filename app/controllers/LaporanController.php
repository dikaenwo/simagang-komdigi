...
class LaporanController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				laporan.id LIKE ? OR 
				laporan.nama_mahasiswa LIKE ? OR 
				laporan.file_laporan LIKE ? OR 
				laporan.tanggal_upload LIKE ?
			)";
			$search_params = array(
				"%$text%","%$text%","%$text%","%$text%"
			);
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("laporan.id", ORDER_TYPE);
		}
		if($fieldname){
	...
	function view($rec_id = null, $value = null){
		...
		if($value){
			...
			$db->where("laporan.id", $rec_id);; //select record based on primary key
		}
		$record = $db->getOne($tablename, $fields );
		if($record){
			$page_title = $this->view->page_title = "View  Laporan";
		$this->view->report_filename = date('Y-m-d') . '-' . $page_title;
		$this->view->report_title = $page_title;
		$this->view->report_layout = "report_layout.php";
		$this->view->report_paper_size = "A4";
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("nama_mahasiswa","file_laporan");
		...
		$page_title = $this->view->page_title = "Add New Laporan";
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","nama_mahasiswa","file_laporan");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'nama_mahasiswa' => 'required',
	...
	function delete($rec_id = null){
		...
		$db->where("laporan.id", $arr_rec_id, "in");
